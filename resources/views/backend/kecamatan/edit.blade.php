        ...
        <div class="col-12">
            <div class="page-title-box">
                <div class="d-flex align-items-center justify-content-between">
                    <h4 class="mb-0 font-size-18">Edit Kecamatan: {{ $kecamatan->name }}</h4>
                ...
                <div class="card-body p-4">
                    <form action="{{ route('backend.kecamatan.update', $kecamatan) }}" method="POST" id="form-kecamatan">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="name" class="form-control" value="{{ old('name', $kecamatan->name) }}"
                                required>
                        </div>

                        <label class="form-label d-block mb-3">Data Populasi per Tahun</label>
                        <div id="populasi-container">
                            @foreach($kecamatan->populasi as $index => $pop)
                                <div class="row populasi-row mb-3" data-id="{{ $pop->id }}">
                                    <div class="col-md-5">
                                        <input type="number" name="populasi[{{ $index }}][year]" class="form-control"
                                            value="{{ $pop->year }}" required>
                                    </div>
                                    <div class="col-md-5">
                                        <input type="number" name="populasi[{{ $index }}][amount]" class="form-control"
                                            value="{{ $pop->amount }}" required>
                                    </div>
                                    <div class="col-md-2">
                        ...
                        <div class="d-flex justify-content-end gap-3">
                            ...
                            <button type="submit" class="btn btn-gradient text-white">
                                <i class="mdi mdi-content-save me-1"></i> Update Kecamatan
                            </button>
                        </div>
                    </form>
                </div>
...
@push('script')
    <script>
        let rowIndex = {{ $kecamatan->populasi->count() }};
        ...
        document.addEventListener('click', function (e) {
            if (e.target.classList.contains('remove-row')) {
                const row = e.target.closest('.populasi-row');
                if (document.querySelectorAll('.populasi-row').length > 1) {
                    row.remove();
                } else {
                    Swal.fire('Minimal harus ada 1 data populasi', '', 'warning');
                }
            }
        });
    </script>
