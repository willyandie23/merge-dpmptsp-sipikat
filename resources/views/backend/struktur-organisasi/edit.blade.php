        ...
        <div class="col-12">
            <div class="page-title-box">
                <div class="d-flex align-items-center justify-content-between">
                    <h4 class="mb-0 font-size-18">Edit Struktur Organisasi</h4>
        ...
        <div class="col-12">
            <div class="card card-modern">
                <div class="card-header card-header-modern">
                    <h4 class="card-title mb-0">Edit Data Anggota Struktur Organisasi</h4>
                </div>

                <div class="card-body p-4">
                    <form action="{{ route('backend.struktur-organisasi.update', $struktur_organisasi) }}" method="POST"
                        enctype="multipart/form-data" id="formEditStruktur">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="id_bidang" id="id_bidang" class="form-select" required>
                                        ...
                                        @foreach($bidangs as $bidang)
                                            <option value="{{ $bidang->id }}" {{ $struktur_organisasi->id_bidang == $bidang->id ? 'selected' : '' }}>
                                                {{ $bidang->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-8">
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="name" class="form-control"
                                        value="{{ old('name', $struktur_organisasi->name) }}" required>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="nip" class="form-control"
                                        value="{{ old('nip', $struktur_organisasi->nip) }}">
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="golongan" class="form-control"
                                        value="{{ old('golongan', $struktur_organisasi->golongan) }}">
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="mb-3">
                        ...
                        @if($struktur_organisasi->image)
                            <div class="mb-4">
                                ...
                                <img src="{{ asset('storage/' . $struktur_organisasi->image) }}"
                                    class="preview-image current-image" alt="Foto Saat Ini">
                            </div>
                        @endif
                        ...
                        <div class="mt-4">
                            <button type="submit" class="btn btn-warning px-4" id="btn-submit">
...
@push('script')
    <script>
        ...
        // Cek status pejabat
        function checkPejabatStatus() {
            const bidangId = $('#id_bidang').val();
            const isCurrentlyPejabat = {{ $struktur_organisasi->is_pejabat ? 'true' : 'false' }};
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.has_pejabat && !isCurrentlyPejabat) {
                        // Bidang sudah punya pejabat, dan data ini BUKAN pejabat utama
                        warning.removeClass('d-none');
                        checkbox.prop('disabled', true);        // disable checkbox
                        // submitBtn tetap aktif, karena user hanya edit staff biasa
                        submitBtn.prop('disabled', false);
                    } else {
                        warning.addClass('d-none');
                        checkbox.prop('disabled', false);
        ...
        // Jalankan lagi jika user mengubah bidang
        $('#id_bidang').on('change', function () {
            checkPejabatStatus();
        });

        // Saat checkbox diubah, cek ulang
        $('#is_pejabat').on('change', function () {
            checkPejabatStatus();
        });
    </script>
@endpush
