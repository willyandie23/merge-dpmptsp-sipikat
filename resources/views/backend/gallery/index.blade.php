        ...
        <div class="col-12">
            <div class="page-title-box">
                <div class="d-flex align-items-center justify-content-between">
                    <h4 class="mb-0 font-size-18">Manajemen Galeri</h4>
        ...
        <div class="col-12">
            <div class="card card-modern">
                <div class="card-header card-header-modern d-flex justify-content-between align-items-center">
                    <h4 class="card-title mb-0">Daftar Foto Galeri</h4>
                    <a href="{{ route('backend.gallery.create') }}" class="btn btn-light btn-add-banner">
                        <i class="mdi mdi-plus me-1"></i> Tambah Foto Baru
                    </a>
                </div>
                            ...
                            <tbody>
                                @forelse ($galleries as $gallery)
                                    <tr>
                                        ...
                                        <td>
                                            <span class="badge rounded-pill px-3 py-2 bg-{{ $gallery->is_active ? 'success' : 'danger' }}">
                                                {{ $gallery->is_active ? 'Aktif' : 'Nonaktif' }}
                                            </span>
                                        </td>
                                        <td class="aksi-column">
                                            <div class="btn-group">
                                                <a href="{{ route('backend.gallery.edit', $gallery) }}" class="btn btn-sm btn-warning">
                                                    <i class="mdi mdi-pencil"></i>
                                                </a>
                                                <button type="button" class="btn btn-sm btn-danger delete-gallery"
                                                        data-id="{{ $gallery->id }}"
    ...
    <script>
        ...
        document.querySelectorAll('.delete-gallery').forEach(button => {
            button.addEventListener('click', function(e) {
                ...
                const title = this.getAttribute('data-title') || 'foto ini';
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        form.method = 'POST';
                        form.action = '{{ route("backend.gallery.destroy", ":id") }}'.replace(':id', id);
